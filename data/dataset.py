...
class EkstraDataset(Dataset):
    ...
    def __init__(self, behavior: pl.DataFrame, history: pl.DataFrame, 
        ...
        sort_pos = [art_idx_pos[art] for art in arte_pos_idx]
        out_of_order = False
    ...
    def get_article_list(self, col, articles_list: List[int]) -> List[Any]:
        return self.articles[col, articles_list]#.tolist()
    
    def get_article_emb_average(self, embs: torch.Tensor, idx_list: List[List[int]]) -> torch.Tensor:
        art_embs = []
        for idx in idx_list:
            art_embs.append(torch.mean(embs[idx, ...], dim=0))
        art_embs = torch.stack(art_embs)
        return art_embs
...
def history_collate(data):
    articles, article_delta_time, impression_weekday, impression_hour = tuple(zip(*data))
    _, articles, article_delta_time, impression_weekday, impression_hour = article_collate(articles, article_delta_time,  impression_weekday, impression_hour)
    return articles, article_delta_time, impression_weekday, impression_hour


def ekstra_inference_collate(data):
    idx, behavior, history = tuple(zip(*data))
    return idx, behavior_collete(behavior), history_collate(history)
    

def ekstra_train_collate(data):
    inference, labels = tuple(zip(*data))
    arts, scrolls = tuple(zip(*labels))
    return ekstra_inference_collate(inference), (arts, scrolls)
